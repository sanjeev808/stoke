...
import { getusersNotification } from "../../store/Notification/actions"
...
const Notifications = () => {
  ...
  const notificationsData = useSelector(state=>state?.UserNotificationlist)
      console.log("get Notification user data", notificationsData?.users?.notificationUser) 
  // const notificationsData = useSelector(notificationsList);
  const [selected, setSelected] = useState("");

  useEffect(() => {
    if (authToken) {
      dispatch(getusersNotification({ authToken }));
    }
  }, [dispatch, authToken]);
  ...
  const updateData = async (row) => {
    ...
    await dispatch(getusersNotification({ authToken }));
  };
            ...
            <Col lg="12" md="12">
              <div className="userbox notification-box position-relative add-newuser">
                ...
                <BootstrapTable
                  ...
                  responsive={true}
                  bordered={true}
                  data={  notificationsData?.users?.notificationUser|| []}
